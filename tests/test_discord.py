...
class FakeChannel:
    ...
    async def send(self, content):
        self.sends += 1
        m = FakeMessage(len(self.messages) + 1)
        m.content = content
        self.messages.append(m)
        return m

    async def edit(self, message, content=None):
        self.edits += 1
...
def test_short_reply_edits_placeholder():
    ...
    out = run(discord_bot.stream_to_channel(ch, events, edit_interval=0))
...
def test_empty_stream_leaves_placeholder():
    ch = FakeChannel()
    out = run(discord_bot.stream_to_channel(ch, [{"type": "done"}], edit_interval=0))
    assert out == ""
