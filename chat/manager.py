...
class ChatManager:
    ...
    def _run_tool_loop(
        ...
        for _ in range(limit):
            ...
            try:
                ...
                # Close now rather than waiting for GC: on interrupt the
                # backend must stop producing tokens immediately.
            ...
            events, records = self._execute_tool_calls(
                turn_tools, turn_content, api_messages, timeline, max_tool_chars, is_cancelled=_interrupted
            )
    ...
    def _claim_active(self, session_id: str) -> dict:
        """Register this turn as the session's active generation.

        A newer turn supersedes any previous one: its cancel flag is set so
        the old generator stops at its next event (see :meth:`run`) without
        saving a partial assistant reply.
        """
        with self._active_lock:
            prev = self._active_runs.get(session_id)
            if prev is not None:
                prev["cancelled"] = True
                log.info("interrupting active generation for session %s", session_id)
