...
def _pick_subtitle(info: dict, lang: str) -> tuple[dict, str]:
    ...
    Preference:
      1. Manual subtitles in ``lang``
      2. Auto captions in ``lang``
      3. Manual subtitles whose code matches the base language
      4. Auto captions whose code matches the base language
    ...
    # Base-language match, manual first.
    for source, table in (("manual", manual), ("auto", auto)):
    ...
    raise YoutubeError(
        f"no subtitles for language '{lang}'. "
        f"Available: {', '.join(available) or '(none)'}"
    )


def _first_fetchable(entries: list[dict]) -> dict | None:
    """Return the first subtitle entry that has a usable URL."""
...
def _parse_vtt(text: str) -> str:
    ...
    for raw in text.splitlines():
        ...
        if not line or line.upper().startswith(
            ("WEBVTT", "KIND:", "ALIGN:", "LANGUAGE:", "STYLE")
        ):
            continue
...
def _download_subtitle(url: str) -> str:
    ...
    No requests/urllib/httpx/etc. is used here. The URL comes from yt-dlp's
    extractor, and the same yt-dlp networking stack is used to retrieve it.
    """
    import yt_dlp
    ...
    try:
        ...
        raise YoutubeError(
            f"could not download subtitle file: {e}"
        ) from e
...
def fetch_transcript(
    ...
    # Download through yt-dlp rather than requests.
    ...
    if not text:
        raise YoutubeError(
            "subtitle file was empty after parsing"
        )
    ...
    if cfg is not None:
        try:
            channel_name = _safe_name(
                channel,
                "unknown-channel",
                60,
            )

            title_name = _safe_name(
                title,
                "untitled",
                100,
            )
            ...
            log.warning(
                "could not save transcript: %s",
                e,
            )

    header = (
        f"Video: {title}\n"
        f"Transcript source: {source} ({lang})"
    )
...
def build_youtube_tools(cfg) -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "Fetch the transcript of a YouTube video. Uses manual "
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "lang": {
                        "type": "string",
                        "description": (
                            "subtitle language code, e.g. 'en', 'de', "
                            "'ja' (default 'en')"
                        ),
                    },
                },
            ...
            handler=lambda a: fetch_transcript(
                a["url"],
                a.get("lang", "en"),
                cfg=cfg,
            ),
