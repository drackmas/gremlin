...
function applySettings(s) {
  ...
  $("set-piper-tts").checked = !!s.piper_tts_enabled;
...
async function loadTtsVoices() {
  const sel = $("set-piper-voice");
  if (!sel) return;
  try {
    const voices = await api("/api/tts/voices");
    sel.innerHTML = "";
    for (const v of voices) {
      const opt = document.createElement("option");
      opt.value = v.id;
      opt.textContent = v.label + (v.available ? "" : " (files missing)");
      sel.appendChild(opt);
    }
    if (state.settings && state.settings.piper_voice) sel.value = state.settings.piper_voice;
  } catch (e) {
    console.error(e);
  }
}
...
async function sendMessage() {
  ...
  // Sending while streaming interrupts the previous turn: abort its fetch,
  ...
  if (state.settings && state.settings.piper_tts_enabled) tts.start();
  else tts.stop();
  input.placeholder = "Gremlin is thinking…  (send a message to interrupt)";
  ...
  try {
    ...
    if (abort.signal.aborted) {
      // Interrupted by Stop or a newer message: no error UI, TTS already stopped.
      refs.bubble.appendChild(el("div", "msg-interrupted text-body-secondary small", "interrupted"));
      if (state.chatAbort !== abort) return; // superseded: the new turn owns shared state
      // Stop (no newer turn): fall through to the shared reset below.
    } else {
  ...
  if (state.chatAbort !== abort) return; // superseded by a newer turn
